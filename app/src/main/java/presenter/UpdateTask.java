...
class UpdateTask extends AsyncTask<Uri, Void, Integer> {
    ...
    UpdateTask(UIInterface userInterface, TaskPresenter presenter) {
...
//        inserting the memory with the proper column name inside the ContentValues
        ...
        int numRows = userInterface.getUIContentResolver().update(updateUri, cv, null, null);
        if (numRows == 1) {
            return numRows;
